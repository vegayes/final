...
if(imageInput != null){ // 화면에 imageInput이 있을 경우 ( if 굳이 안해도 되긴 함 ) 
    ...
    originalImage = profileImage.getAttribute("th:src"); 

    // 회원 프로필 화면 진입 시 
    // 현재 회원의 프로필 이미지 상태를 확인
    if(profileImage.getAttribute("th:src") == "@{/images/admissionApply/강아지.png}"){
        initCheck = false;
    }else{
        initCheck = true;
    }
    ...
    imageInput.addEventListener("change", e => {
        ...
        if(file == undefined){ 
            ...
            profileImage.setAttribute("th:src", originalImage);
        ...
        reader.onload = e => {
            ...
            profileImage.setAttribute("th:src", url);
    ...
    deleteImage.addEventListener('click', () => {
        ...
        profileImage.setAttribute("th:src", "@{/images/admissionApply/강아지.png}");
        // 프로필 이미지를 기본 이미지로 변경
