...
public class MemberUserServiceImpl implements MemberUserService{
	...
	public int updateInfoUser(MultipartFile profileImage, Member updateMember, Member loginMember) throws Exception {
		...
		String strPw = String.valueOf(updateMember.getMemberPw());
		...
		if(profileImage.getSize() > 0) { // 업로드된 이미지가 있을 경우
			...
			// 프로필 이미지가 있는데 그냥 수정 버튼 누르면 이미지가 없어짐
			// 프로필 이미지 변경여부를 체크하여 값이 'N'이면 updateMember.setMemberProfile(null);
			String updateProfileYn = updateMember.getUpdateProfileYn();
			
			if(updateProfileYn != "N") { 
